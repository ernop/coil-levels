...
def main():
    ...
    lines+=['','## Coverage and limitations','','The first survey holds the candidate limit at 20 for all 53 originally selectable tweak names. Six tweak families are crossed with the other six segment pickers. Three tweak families receive seven extra control settings. This is not all 53 × 7 × all limits × all seed values.','','The original unbounded full-length initial walk could spiral across most of the board before tweaking. All nine affected survey boards now use eight full slides; the measurements here describe the repaired outputs. Run distributions, pillars, axis bias, and spatial density still expose differences among the remaining styles. Use the visual stats lab to inspect what each statistic counts and what it misses. Crop images carry their labels and coordinates on the image itself. See REPAIRS.md for the before/after audit.','','The 500-square unlimited-last exports took 51.6 and 122.2 seconds. The 1000-square attempt was stopped before export after several minutes; no board is claimed for it. Larger selected boards replace that recipe with `seg-2lim10-Last` (lim20), chosen by the same geometric coverage rule among finite-candidate alternatives.','','Next investigations: more seeds for high-spread configurations; more initial-walk lengths and starting positions; controlled spatial maps and motif spectra; bridge/room/proof analysis on selected manageable boards. These remain distinct from a ranking by search effort.','']
    ...
    lines += ['', 'Budget-exceeded attempts have no claimed specimen. The retained larger collection uses additional Weighted4/lim20 representatives selected for measured geometry coverage. The 10000-square series uses rnd99, eight initial full slides at two seeds, and len23. Generation budgets measure practicality in these runs, not mathematical impossibility or solver difficulty.', '']
    ...
    lines += ['', 'Compare the repaired eight-slide seed pair to inspect variation within one method at 10000 square. Whole-board overviews and explicitly labeled cell crops serve different scales of inspection. The current 54th picker, len23-10th-exact, is not retroactively added to the frozen 110-configuration survey.', '']
